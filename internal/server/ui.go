...
async function load(){var r=await fetch(A+'/packages').then(function(r){return r.json()});items=r.packages||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.version||'').toLowerCase().includes(q)||(i.registry||'').toLowerCase().includes(q)||(i.checksum||'').toLowerCase().includes(q)});
...
if(i.registry)h+='<span>'+esc(i.registry)+'</span>';
...
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/packages/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:"",version:"",registry:"",checksum:"",size_bytes:0,downloads:0,status:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' PARCEL</h2>';
...
h+='<div class="fr"><label>Registry</label><input id="f-registry" value="'+esc(i.registry)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),version:document.getElementById('f-version').value.trim(),registry:document.getElementById('f-registry').value.trim(),checksum:document.getElementById('f-checksum').value.trim(),size_bytes:parseInt(document.getElementById('f-size-bytes').value)||0,downloads:parseInt(document.getElementById('f-downloads').value)||0,status:document.getElementById('f-status').value.trim()};
if(editId){await fetch(A+'/packages/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/packages',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
